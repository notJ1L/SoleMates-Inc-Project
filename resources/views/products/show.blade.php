...
@section('title', $product->name . ' — SoleMates Footwear')
...
@php
                    ...
                    <div class="thumb-strip">
                        @foreach($allPhotos as $i => $photo)
                            <div class="thumb {{ $i === 0 ? 'active' : '' }}"
                                 onclick="switchPhoto(this, '{{ $photo->src }}')">
        ...
        {{-- ===== PRODUCT DETAILS ===== --}}
        <div class="col-lg-7">
            <div class="product-detail-brand">{{ $product->brand->name ?? 'SoleMates' }}</div>
            ...
            <div class="product-meta">
                ...
                <div class="meta-row">
                    ...
                    <a href="{{ route('products.index') }}?category={{ urlencode($product->category->name ?? '') }}"
                       style="color:var(--accent); text-decoration:none;">{{ $product->category->name ?? 'Uncategorized' }}</a>
                </div>
                @endif
                @if($product->size)
                <div class="meta-row">
...
@php $relThumb = $related->thumbnailUrl(); @endphp
                        @if($relThumb)
                            <img src="{{ $relThumb }}" alt="{{ $related->name }}">
                        @else
